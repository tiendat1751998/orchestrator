# Micro-Task 1.27: Tạo contracts/workflow/workflow.go

## Thông tin
- **File tạo**: `contracts/workflow/workflow.go`
- **Package**: `workflow`
- **Dependencies trước**: 1.07 (contracts/status.go), 1.18 (agent/task.go — cho kiến trúc tham khảo)
- **Thời gian**: 10 phút
...
type Step struct {
	...
	// If empty, the step always executes (when dependencies are met).
	Condition string `yaml:"condition,omitempty" json:"condition,omitempty"`

	// OnFailure defines behavior when this step fails.
	// Values: "retry" (retry MaxRetries times), "skip" (continue), "abort" (stop workflow)
	// Default: "abort"
...
1. **Workflow vs Mission**: Workflow = human-defined steps (YAML). Mission = AI-planned tasks. Khác nhau hoàn toàn.
2. **OnFailure default**: Nếu không set → "abort" (fail fast). Phải document rõ default behavior.
3. **Condition expression**: Phase 1 chỉ định nghĩa field. Expression engine implement ở Phase 5.

## Checklist
- [ ] File `contracts/workflow/workflow.go` tồn tại
- [ ] Package: `package workflow`
- [ ] Import `contracts` cho Status type
- [ ] Workflow interface với 3 methods
- [ ] Step struct với 6 fields
- [ ] Result struct với 3 fields
- [ ] StepResult struct với 4 fields
- [ ] YAML + JSON tags cho Step
- [ ] Godoc comments
- [ ] `go build ./contracts/workflow/...` không lỗi
