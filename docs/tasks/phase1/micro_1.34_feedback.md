# Micro-Task 1.34: Tạo contracts/feedback/feedback.go

## Thông tin
- **File tạo**: `contracts/feedback/feedback.go`
- **Package**: `feedback`
...
1. **Score range 0-1**: Consistent across evaluators. KHÔNG dùng 0-100 (mixing ranges = bugs).
2. **RecordResult thread-safety**: Multiple agents complete tasks concurrently. Scorer MUST use mutex.
3. **AI-as-judge cost**: Evaluating with AI adds another provider call. Use sparingly. Default to heuristics.
4. **Cold start**: GetScore() returns 0.0 when no data → planner falls back to default agent selection.

## Checklist
- [ ] File `contracts/feedback/feedback.go` tồn tại
- [ ] Package: `package feedback`
- [ ] Evaluator interface với 1 method
- [ ] EvalInput struct với 4 fields
- [ ] EvalResult struct với 3 fields
- [ ] Scorer interface với 3 methods
- [ ] AgentScore struct với 4 fields
- [ ] Score range documented (0-1)
- [ ] Godoc comments
- [ ] `go build ./contracts/feedback/...` không lỗi
