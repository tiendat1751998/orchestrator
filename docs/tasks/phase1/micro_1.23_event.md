# Micro-Task 1.23: Tạo contracts/event/event.go

## Thông tin
- **File tạo**: `contracts/event/event.go`
- **Package**: `event`
...
1. **Subscribe trả về `unsubscribe func()`**: Consumer PHẢI gọi → tránh goroutine/memory leak. Pattern: `unsub, _ := bus.Subscribe(...); defer unsub()`.
2. **Payload là `any`**: Mỗi event type có payload khác nhau. Consumer dùng type assertion: `result := event.Payload.(*agent.Result)`.
3. **Handler runs in goroutine**: Bus gọi handler trong goroutine riêng → handler PHẢI thread-safe.
4. **Handler panic recovery**: Bus PHẢI recover() nếu handler panic → tránh crash toàn bộ hệ thống.
5. **Wildcard matching**: `"task.*"` match `"task.started"` nhưng KHÔNG match `"task"` (không có sub-event). Implementation trong Phase 2.

## Checklist
- [ ] File `contracts/event/event.go` tồn tại
- [ ] Package: `package event`
- [ ] Event struct với 5 fields
- [ ] Bus interface với 2 methods (Publish, Subscribe)
- [ ] Subscribe trả về `unsubscribe func()`
- [ ] 10 event type constants
- [ ] `NewEvent()` constructor
- [ ] Godoc comments chi tiết
- [ ] `go build ./contracts/event/...` không lỗi
