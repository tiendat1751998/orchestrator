# Micro-Task 1.30: Tạo contracts/orchestrator/orchestrator.go

## Thông tin
- **File tạo**: `contracts/orchestrator/orchestrator.go`
- **Package**: `orchestrator`
- **Dependencies trước**: 1.07 (status.go), 1.19 (agent/result.go), 1.29 (planner/planner.go)
- **Thời gian**: 10 phút
...
type MissionResult struct {
	...
	// May be generated by AI or from a template.
	Summary string `json:"summary"`

	// Artifacts are all files/outputs produced across all tasks.
	// Merged from individual task results.
...
1. **Import planner**: Orchestrator nhận `*planner.Mission`. OK — orchestrator depends on planner, not the other way.
2. **Import agent**: Orchestrator returns `*agent.Result`. OK — same direction.
3. **Status() thread-safety**: Called from CLI goroutine while ExecuteMission runs on another goroutine. Underlying data MUST use mutex or atomic operations.
4. **Progress() division by zero**: Check `TotalTasks == 0`.

## Checklist
- [ ] File `contracts/orchestrator/orchestrator.go` tồn tại
- [ ] Package: `package orchestrator`
- [ ] Import `contracts`, `contracts/agent`, `contracts/planner`
- [ ] Orchestrator interface với 3 methods
- [ ] MissionResult struct với 6 fields
- [ ] MissionStatus struct với 7 fields
- [ ] `Progress()` method with division-by-zero guard
- [ ] Godoc comments
- [ ] `go build ./contracts/orchestrator/...` không lỗi
