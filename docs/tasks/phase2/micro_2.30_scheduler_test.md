...
## Purpose
Tests for priority queue, dependency tracker, and scheduler integration.
...
func TestPriorityQueue_HigherPriorityFirst(t *testing.T) {
	...
	// Should come out in order: high(10), mid(5), low(1)
	task1 := q.Dequeue()
	if task1.ID != "high" {
...
func TestPriorityQueue_DequeueWait_GetsSignaled(t *testing.T) {
	...
	// Enqueue after the goroutine is waiting
...
func TestDependencyTracker_CircularDependency_ThreeNodes(t *testing.T) {
	...
	err := dt.AddDependency("C", "A") // A→B→C→A = cycle
	if err == nil {
...
func TestDependencyTracker_DiamondDependency(t *testing.T) {
	...
	// D still not ready (B and C not completed)
	if dt.IsReady("D") {
...
func TestScheduler_SubmitAndRun(t *testing.T) {
	...
	sched := scheduler.New(
		...
		nil, // nil logger for test
	)

	// Submit 3 tasks with no dependencies
	for i := 0; i < 3; i++ {
		task := &agent.Task{
	...
	// Run scheduler loop with timeout
...
func TestScheduler_DependencyWait(t *testing.T) {
	...
	sched.Submit(taskB, 10) // Higher priority but has dependency
	sched.Submit(taskA, 1)  // Lower priority but no dependency
	...
	// Notify A completed → B becomes ready
	...
	// A should be dispatched first (it's ready), then B (after A completes)
	if len(dispatched) < 2 {
...
func TestScheduler_CircularDependencyRejected(t *testing.T) {
	...
	err := sched.Submit(taskB, 1) // Should fail: A→B, B→A = cycle
	if err == nil {
...
## Verify
```bash
go test -v -race -count=1 ./kernel/scheduler/...
...
- [ ] Package: `scheduler_test`
- [ ] ≥ 20 test functions
- [ ] Queue: priority ordering, FIFO, empty dequeue, wait+cancel, wait+signal
- [ ] Queue: Len, Remove, Peek, concurrent access
- [ ] Deps: no deps, single dep, multiple deps, self-dep, circular (2-node, 3-node)
- [ ] Deps: PendingDependencies, diamond dependency
- [ ] Scheduler: submit+run, dependency wait, duplicate submit, circular rejection
- [ ] Scheduler: QueueLen, PendingLen
- [ ] Uses atomic counters for concurrent assertions
- [ ] `go test -v -race ./kernel/scheduler/...` ALL PASS
