# Micro-Task 2.04: Tạo kernel/config/loader.go

## Thông tin
- **File tạo**: `kernel/config/loader.go`
- **Package**: `config`
- **Dependencies trước**: 2.01 (config.go), 2.02 (defaults.go), 2.03 (env.go)
- **Thời gian**: 20 phút
- **Verify**: `go build ./kernel/config/...`

## Mục đích
Load config từ YAML file, resolve env vars, merge defaults, parse durations.
Đây là entry point duy nhất cho config loading.
...
### Pitfall 1: time.Duration YAML parsing
```yaml
timeout: "120s"   # yaml.v3 thấy đây là string "120s", KHÔNG phải time.Duration
timeout: 120s     # yaml.v3 thấy đây là string "120s" (unquoted)
timeout: 120      # yaml.v3 thấy đây là int 120 (nanoseconds!) — SAI
```
Giải pháp: intermediate struct với `string` fields → `time.ParseDuration()`.

### Pitfall 2: Double YAML parse (marshal/unmarshal)
Tại sao parse 2 lần?
1. Lần 1: `yaml.Unmarshal(data, &raw)` → raw `map[string]any` → resolve env vars
2. `yaml.Marshal(raw)` → bytes (với env vars đã resolved)
3. Lần 2: `yaml.Unmarshal(resolvedData, &intermediate)` → typed struct

Nếu chỉ parse 1 lần → env vars CHƯA resolve khi unmarshal vào typed struct.

### Pitfall 3: No config file = NOT an error
`LoadFromSearchPaths()` trả về `DefaultConfig()` nếu không tìm thấy file.
Người dùng mới chạy orchestrator lần đầu chưa có config → vẫn hoạt động.

### Pitfall 4: Error messages phải rõ ràng
```go
// ❌ SAI:
return nil, err

// ✅ ĐÚNG:
return nil, fmt.Errorf("config: providers.%s.timeout %q: %w", name, ip.Timeout, err)
// Output: "config: providers.antigravity.timeout "abc": time: invalid duration "abc""
```
User biết CHÍNH XÁC field nào sai, giá trị sai là gì.

### Pitfall 5: ParseBytes cho testing
`ParseBytes()` public function cho phép test config parsing mà không cần file system.
```go
func TestParseConfig(t *testing.T) {
    cfg, err := config.ParseBytes([]byte(`...yaml...`))
    // test cfg
}
```

## Checklist
- [ ] File `kernel/config/loader.go` tồn tại
- [ ] Package: `package config`
- [ ] `Load(path)` function — entry point từ file path
- [ ] `LoadFromSearchPaths()` — auto-find config file
- [ ] `parseConfig(data)` — internal pipeline
- [ ] `intermediateConfig` struct cho duration parsing
- [ ] `toConfig()` conversion với `time.ParseDuration()`
- [ ] `ParseBytes()` public function cho testing
- [ ] Env var resolution qua `ResolveEnvInMap()`
- [ ] `MergeWithDefaults()` called sau unmarshal
- [ ] Error messages chỉ rõ field path + giá trị sai
- [ ] No config file → returns DefaultConfig (NOT error)
