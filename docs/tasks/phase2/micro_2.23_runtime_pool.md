...
## Purpose
Worker pool with semaphore-based concurrency limiting.
Prevents OOM from unbounded goroutine creation.
...
### Pitfall 1: Semaphore = buffered channel
```go
sem := make(chan struct{}, maxWorkers)
// Write to acquire (blocks when buffer full):
sem <- struct{}{}
// Read to release:
<-sem
```
This is the standard Go pattern for bounded concurrency. No external dependencies needed.

### Pitfall 2: wg.Add(1) BEFORE go func
```go
p.wg.Add(1)      // MUST be before goroutine launch
go func() {
    defer p.wg.Done()
    ...
}()
```
If `wg.Add(1)` is inside the goroutine → `Wait()` may return before `Add(1)` runs → premature completion.

### Pitfall 3: Release semaphore in defer
```go
defer func() {
    <-p.sem          // ALWAYS release, even on panic
    p.wg.Done()      // ALWAYS mark done
}()
```
If `<-p.sem` is not in defer → panic in fn() → slot never released → pool permanently reduced.

### Pitfall 4: Context cancellation while waiting
```go
select {
case p.sem <- struct{}{}:  // Slot available
case <-ctx.Done():          // Cancelled while waiting
    return ctx.Err()
}
```
Without this → Submit blocks forever when pool is full and context is cancelled.

### Pitfall 5: maxWorkers < 1 guard
```go
if maxWorkers < 1 { maxWorkers = 1 }
```
`make(chan struct{}, 0)` creates an unbuffered channel → `sem <- struct{}{}` always blocks → deadlock.

## Checklist
- [ ] File `kernel/runtime/pool.go` exists
- [ ] Package: `package runtime`
- [ ] Pool struct with: sem channel, WaitGroup, atomic counters
- [ ] `NewPool(maxWorkers, logger)` — defaults maxWorkers to 1 if < 1
- [ ] `Submit(ctx, fn)` — blocks when pool full, respects context cancellation
- [ ] `Wait()` — blocks until all workers complete
- [ ] `ActiveWorkers()` — atomic counter
- [ ] `MaxWorkers()` — returns configured max
- [ ] `Stats()` — returns PoolStats struct
- [ ] Semaphore released in defer (panic-safe)
- [ ] wg.Add(1) before goroutine launch
- [ ] Context cancellation in Submit
