# Micro-Task 5.16: Create kernel/resilience/retry.go
...
## Purpose
Implements the retries scheduler (`Retry` and exponential backoff configuration engines) with randomized delay jittering to prevent thundering herd calls during outages.
...
### Pitfall 1: Retrying non-retryable authentication failures
```go
// WRONG:
if err != nil {
    // Retries immediately without validation check: triggers 5 calls on bad API keys!
}

// CORRECT:
if !IsRetryable(err) {
    return err // Quit immediately on bad credentials (401/403)
}
```
Attempting to retry authentication errors wastes resources and API limits. Filter error types and only retry transient network/rate limit issues.

### Pitfall 2: Blocking context cancellations during backoff delays
Using `time.Sleep(delay)` inside retry loops blocks context cancellations. Always use `select` blocks with `<-ctx.Done()` and `<-time.After(delay)`.
...
## Checklist
- [ ] File exists at `kernel/resilience/retry.go`
- [ ] Package name is `resilience`
- [ ] All exported types have Godoc
- [ ] Authentication errors skip retries
- [ ] Delays calculate exponential multipliers with jitter offsets
- [ ] Backoff waits monitor context cancel triggers
