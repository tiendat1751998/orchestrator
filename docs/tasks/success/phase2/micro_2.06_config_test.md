# Micro-Task 2.06: Create kernel/config/config_test.go
...
## Purpose
Implements comprehensive unit tests for configuration parsing, default values merging, validation checks, environment variable resolution, and duration parsing.
...
## Rules
1. **Isolated Envs**: Use `t.Setenv()` rather than `os.Setenv` to set environment variables. This prevents interference with parallel test suites and runs cleanups automatically.
2. **Directory Isolation**: Use `t.TempDir()` rather than legacy temporary directory tools to ensure isolation.
3. **Type assertion safety**: Always verify type assertions (`verrs, ok := err.(*config.ValidationErrors)`) to avoid panics.

## ⚠️ Pitfalls

### Pitfall 1: Modifying global state configurations inside unit tests
Modifying global environment variables using `os.Setenv` causes thread collisions and breaks other concurrently executing tests. Always use `t.Setenv` to scope variables to the lifetime of the running test block.

### Pitfall 2: Panicking on type assertion failures in assertions
Directly asserting error structures (`err.(*config.ValidationErrors)`) will crash the test process if a different error type (such as a parsing or file read error) is returned. Always use the two-value type assertion syntax to handle casting errors cleanly.

## Verify
```bash
go test -v -race ./kernel/config/...
```

## Checklist
- [ ] File `kernel/config/config_test.go` exists
- [ ] Package: `config_test`
- [ ] Temporary YAML helper function `writeTempYAML` declared
- [ ] At least 15 test functions are implemented
- [ ] Tests verify default values, env resolution, and merge steps
- [ ] Tests assert structured validation outcomes
- [ ] `t.Setenv` and `t.TempDir` are used for environment settings and files isolation
- [ ] `go test -v -race ./kernel/config/...` passes
