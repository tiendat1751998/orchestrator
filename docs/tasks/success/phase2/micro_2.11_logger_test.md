# Micro-Task 2.11: Create kernel/logger/logger_test.go

## Info
- **File**: `kernel/logger/logger_test.go`
- **Package**: `logger_test`
- **Depends on**: 2.07-2.10
- **Time**: 20 min
- **Verify**: `go test -v -race ./kernel/logger/...`

## Purpose
Implements unit tests verifying the correctness of JSON/Text formats, log level filtering, sub-loggers, security redaction, and duration formatting in the logger package.
...
## Rules
1. **External Test Scope**: Enforce package declaration `logger_test` to verify logger capabilities behave correctly as imported by consumers.
2. **Buffer Isolation**: Write log outputs to local memory buffer structures (`bytes.Buffer`) to verify printed strings, avoiding writing to standard OS files in tests.
3. **No Side Effects**: Verify that copy functions (such as `RedactMap`) create new maps instead of mutating inputs.

## ⚠️ Pitfalls

### Pitfall 1: Bypassing type checking when checking logs contents
Do not rely on exact matches if fields contain varying white spaces or JSON indentation. Check matching attributes using substrings (`strings.Contains`).

### Pitfall 2: Mutating input variables under test assertions
Ensure assertions verify that `original` maps are not modified after running through `RedactMap` to prevent state corruption side effects in operations.

## Verify
```bash
go test -v -race ./kernel/logger/...
```

## Checklist
- [ ] File `kernel/logger/logger_test.go` exists
- [ ] Package: `logger_test`
- [ ] At least 15 test functions are declared
- [ ] Tests verify defaults, JSON formats, Text formats, and levels filtering
- [ ] Tests verify sub-loggers attributes
- [ ] Tests verify exact and suffix key redactions
- [ ] `RedactMap` copies rather than mutating inputs
- [ ] `FormatDuration` outputs milliseconds, seconds, and minutes correctly
- [ ] `go test -v -race ./kernel/logger/...` passes
