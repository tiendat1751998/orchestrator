# Micro-Task 2.01: Create kernel/config/config.go

## Info
- **File**: `kernel/config/config.go`
- **Package**: `config`
- **Depends on**: Phase 1 completed (contracts packages)
- **Time**: 20 min
- **Verify**: `go build ./kernel/config/...`

## Purpose
Defines the Go structures that map 1:1 with the orchestrator YAML configuration schema. This file does not contain loading or validation logic, only structural specifications.

## External dependencies to add
```bash
go get gopkg.in/yaml.v3
```
After adding dependencies, run `go mod tidy` to clean up the workspace.
...
## Rules
1. **Config Struct Passing**: The `Config` struct must be passed by pointer using dependency injection. Never declare a global `Config` instance.
2. **Hidden Secrets**: The `APIKey` field must carry `json:"-"` to prevent printing secrets in HTTP REST API config endpoints.
3. **Decoupled Imports**: The configuration structures must remain separated from system log packages. Express log levels as simple string fields (`log_level: "info"`) rather than `slog.Level` types.

## Pitfalls

### Pitfall 1: Relying on global variables for settings configuration
```go
// WRONG:
var GlobalConfig *Config // Shared state causes race conditions and blocks test execution suites.

// CORRECT:
func NewKernel(cfg *Config) *Kernel { ... } // Dependency injection pattern
```
Global variables couple packages together and make concurrent testing impossible. Always pass config pointers explicitly through constructors.

### Pitfall 2: Failing to parse human-readable durations in yaml.v3
The default Go YAML package `gopkg.in/yaml.v3` does not support converting duration strings like `"30s"` or `"120s"` directly into `time.Duration` fields, causing unmarshal errors. We will address this by writing a custom YAML unmarshaling helper in our configuration loader file.
...
## Checklist
- [ ] File `kernel/config/config.go` exists
- [ ] Package: `config`
- [ ] `Config` contains Orchestrator, Providers, Agents, and Security top-level configs
- [ ] `OrchestratorConfig` declares Name, LogLevel, LogFormat, DataDir, MaxConcurrentTasks, and ShutdownTimeout
- [ ] `ProvidersConfig` declares Default and Configs maps
- [ ] `APIKey` is marked with `json:"-"`
- [ ] `Temperature` and `MaxTokens` overrides use pointers (`*float64`, `*int`)
- [ ] Minimal imports exist (no log dependencies)
