...
## Purpose
Implements the task dependency tracker (`DependencyTracker` and constructors) that records parent-child dependencies, detects circular cycles via DFS traversals, rolls back invalid configurations, and checks task readiness.
...
## Rules
1. **Circular Dependency Guards**: Perform cycle detection Checks using DFS before finalizing dependency registrations. If a cycle is detected, return an error and roll back the changes to keep the graph in a consistent state.
2. **Lock Optimization**: Optimize locking by using write locks (`Lock()`) for updates (such as adding dependencies) and read locks (`RLock()`) for queries (such as checking readiness).
3. **DFS Recursion Safeties**: Always use a `visited` tracking map inside DFS recursion methods to avoid infinite loop crashes on diamond dependency layouts (e.g. A → B → D and A → C → D).

## ⚠️ Pitfalls

### Pitfall 1: Permitting circular dependencies in graph updates
Allowing dependencies like A → B and B → A creates a deadlock where neither task can ever start. Ensure you validate graphs using the `hasCircularDep` check during registration.

### Pitfall 2: Omiting visited maps inside DFS graph checks
In diamond dependency structures (e.g., A depends on B and C; B and C both depend on D), executing DFS traversals without tracking visited nodes leads to repeated checks and stack overflow panics. Always track visited paths.
...
- [ ] Package: `scheduler`
- [ ] `DependencyTracker` encapsulates dependencies and completion maps
- [ ] `AddDependency` verifies self-dependency limits and checks cycles
- [ ] DFS traversals roll back additions when cycles are found
- [ ] `IsReady` returns true if a task has no dependencies or all are completed
- [ ] `PendingDependencies` lists uncompleted dependency IDs
- [ ] `Reset` clears tracker memory between mission runs
- [ ] locks use `sync.RWMutex`
