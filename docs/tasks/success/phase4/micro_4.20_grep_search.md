# Micro-Task 4.20: Create plugins/tools/filesystem/search.go
...
## Purpose
Implements the file contents search tool (`SearchTool` and schemas) to locate text matching target strings inside workspace text files, enforcing results caps.
...
### Pitfall 1: Walking large project folders recursively without skipping build tools
Walking files recursively through build folders like `node_modules` or `.git` metadata blocks consumes massive memory and CPU. Exclude these directories.

### Pitfall 2: Blocking traversals during context cancellations
If searching a large workspace takes a long time, failing to verify `ctx.Done()` inside directory walks can cause search commands to hang. Check context states.
...
## Checklist
- [ ] File exists at `plugins/tools/filesystem/search.go`
- [ ] Package name is `filesystem`
- [ ] All exported types have Godoc
- [ ] Traversal loops skip `.git`, `node_modules`, `vendor`, and `.gemini` paths
- [ ] Total matches are capped at 50 results
