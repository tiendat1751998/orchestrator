# Micro-Task 4.23: Create plugins/tools/tools_test.go

## Info
- **File**: `plugins/tools/tools_test.go`
- **Package**: `tools_test`
- **Depends on**: 4.17, 4.18, 4.19, 4.20, 4.21, 4.22
- **Time**: 20 min
- **Verify**: `go test -v -race -count=1 ./plugins/tools/...`

## Purpose
Implements integration unit tests for the core filesystem, terminal, and git execution tools, verifying traversal guards, atomic write steps, security policies, and listing operations.
...
### Pitfall 2: Attempting to assert against exact file structures
System temp directories contain unpredictable subfolders. Assert against relative file entries instead of hardcoded paths.

## Verify
```bash
go test -v -race -count=1 ./plugins/tools/...
```

## Checklist
- [ ] File exists at `plugins/tools/tools_test.go`
- [ ] Package name is `tools_test`
- [ ] All exported types have Godoc
- [ ] Temporary test workspaces are initialized and deleted
- [ ] Write, Read, and List tools verify complete data loops
- [ ] File path traversal attempts are successfully blocked
- [ ] Dangerous shell commands trigger security policy errors
