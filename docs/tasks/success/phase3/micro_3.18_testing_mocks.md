...
## Purpose
Implements mock structures (`MockProvider`, `MockAgent`, `MockTool`, `MockEventBus` and handlers) for integration testing and unit testing, allowing developers to inject test behaviors using function pointers.
...
## Rules
1. **Thread-Safe Mock Tracking**: Guard slice collections (such as `Published` inside `MockEventBus`) using mutex synchronization locks.
2. **Deep Copy Returns**: Return copies of internal collections rather than returning direct memory addresses in `GetPublished()`.
3. **Flexible Function Pointers**: Provide customizable function fields (`SendFn`, `ExecuteFn`) to allow tests to override default mock behavior.

## ⚠️ Pitfalls

### Pitfall 1: Non-synchronized list modifications in mocks
```go
```
Stress testing parallel workflows can access mock state arrays concurrently. Guard them using locking primitives.

### Pitfall 2: Returning mock data references directly
If tests read and modify returned slices directly, they corrupt mock history, causing test assertions to fail. Always return a copied slice.
...
- [ ] Package: `testing`
- [ ] `MockProvider` satisfies provider contracts
- [ ] `MockAgent` satisfies agent contracts
- [ ] `MockTool` satisfies tool contracts
- [ ] `MockEventBus` uses a `sync.RWMutex` to guard state arrays
- [ ] `GetPublished` returns deep copies of tracked lists
- [ ] Mocks accept custom logic overrides using function pointers
