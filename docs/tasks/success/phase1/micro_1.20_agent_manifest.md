# Micro-Task 1.20: Create contracts/agent/manifest.go
...
## Purpose
Defines the `Manifest` struct representing the configuration attributes of an agent plugin.
...
### Pitfall 1: Storing direct Provider/Tool instance pointers in Manifest
```go
Provider string `yaml:"provider"` // Decoupled: stores string key name, resolved by the Registry at runtime.
```
Always use string names in config manifests to decouple plugin definitions from implementation dependencies.

### Pitfall 2: Forgetting either YAML or JSON tags
Agent configuration parameters are parsed from YAML files but exposed as JSON via the orchestrator status endpoints. Ensure all fields carry both `yaml:"..."` and `json:"...,omitempty"` tags.
...
## Checklist
- [ ] File `contracts/agent/manifest.go` exists
- [ ] Package: `agent`
- [ ] `Manifest` contains all 12 configuration fields
- [ ] Every field has both YAML and JSON tags
- [ ] Optional parameters carry the `omitempty` tag
