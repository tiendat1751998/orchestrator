# Micro-Task 1.42: Create contracts/knowledge/knowledge.go

- **File**: `contracts/knowledge/knowledge.go`
- **Package**: `knowledge`
- **Depends on**: 1.06 (types.go)
- **Time**: 15 min
- **Verify**: `go build ./contracts/knowledge/...`

## Purpose
Declares the specialized contracts and interfaces inside the `Knowledge Engine` database layer:
- `KnowledgeStore` (Core entry storage)
- `SkillGraph` (Dependency resolution - RFC-0032)
- `ProductMemory` (Business ontologies - RFC-0041)
- `KnowledgeDecayer` (Depreciation and TTL - RFC-0051)
...
## Verify
```bash
go build ./contracts/knowledge/...
```

## Checklist
- [ ] File `contracts/knowledge/knowledge.go` exists
- [ ] Package: `knowledge`
- [ ] Interfaces declared for `KnowledgeStore`, `SkillGraph`, `ProductMemory`, and `KnowledgeDecayer`
- [ ] `go build ./contracts/knowledge/...` passes
