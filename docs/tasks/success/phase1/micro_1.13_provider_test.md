# Micro-Task 1.13: Create contracts/provider/provider_test.go

## Info
- **File**: `contracts/provider/provider_test.go`
- **Package**: `provider_test`
- **Depends on**: 1.08, 1.09, 1.10, 1.11, 1.12
- **Time**: 20 min
- **Verify**: `go test -v -race ./contracts/provider/...`

## Purpose
Implements unit tests verifying the correctness of JSON serialization, validation gates, usage counters, and config defaults in the Provider contract models.
...
### Pitfall 1: Attempting to assert test coverage from inside target package
Using internal package structure (e.g. `package provider` inside `provider_test.go`) makes it hard to simulate external consumer imports. Always write contract unit tests using the external convention (`package provider_test`) to verify public API exposure.

### Pitfall 2: Reusing tests files across concurrent test processes
Sharing mutable test instances or setting global variables in unit tests risks resource contentions. Keep unit tests isolated and thread-safe.

## Verify
```bash
go test -v -race ./contracts/provider/...
```

## Checklist
- [ ] File `contracts/provider/provider_test.go` exists
- [ ] Package: `provider_test`
- [ ] Test `TestMessage_JSONRoundTrip` verifies serialization roundtrip
- [ ] Test `TestToolCall_ArgsPreservesJSONPrecision` checks raw JSON unmarshal without losing big precision
- [ ] Test `TestRequest_PointerFields_NilVsZero` validates pointer value checks
- [ ] Test `TestConfig_APIKeyNotInJSON` verifies key secrecy
- [ ] `go test -v -race ./contracts/provider/...` passes
