...
> **Mandatory Rules**:
> - Kernel MUST NOT import from `plugins/`. Only import from `contracts/`.
> - Each micro-task = 1 single file, containing EXACT code blocks that can be copied/pasted.
> - Go version: **1.26**
> - Each file must include Godoc comments, error handling, and verification commands.

## Dependency Rule

```
contracts/ ← kernel/ ← plugins/
              ↑ NEVER import plugins/
```
...
| Category | Tasks Count | Estimated Time |
|---|---|---|
| Config | 6 | 110 min |
| Logger | 5 | 75 min |
| EventBus | 6 | 100 min |
...
```
Read file docs/tasks/phase2/micro_2.XX_name.md and implement EXACTLY
what is described. Do not add or remove. Then run verify command.
```
